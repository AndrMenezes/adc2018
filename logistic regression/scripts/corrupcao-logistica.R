...
library(Hmisc);library(reshape2);library(dplyr);library(Amelia)

#---------------------------------------------------------------------------------
# Importando os dados sav ----------------------------------------
dir   <- "C:/Users/User/Dropbox/Observatório 2018/Dados/datafolha_09_2017/04423.SAV" #"/home/furriel/Dropbox/Observatório 2018/Dados/datafolha_09_2017/04423.SAV"
...
table(dados$p18)
dados$corrupcao[dados$p18 == "1"] <- "Corrupcao"
dados$corrupcao[dados$p18 == "2"] <- "Incompetencia"
dados$corrupcao <- dados$corrupcao%>%as.factor()
dados$corrupcao <- relevel(dados$corrupcao, ref = "Incompetencia")
...
levels(dados$p13)   
dados$denunciaTemer[dados$p13 == 1] <- "Sim_deveria"
dados$denunciaTemer[dados$p13 == 2] <- "Nao_deveria"
...
table(dados$p15)
dados$Ljato[dados$p15 == 1] <- "Ira_diminuir"
dados$Ljato[dados$p15 == 2] <- "Ira_aumentar"
dados$Ljato[dados$p15 == 3] <- "Continuara_igual"
...
write.csv(edados, file = "C:/Users/User/Dropbox/5° Série/Análise de Dados Categóricos/Regressão Logística/scripts/corrupcao.csv", 
...
roc <- ggplot(dadosx,aes(d = corrupcao, m = predict)) + 
         geom_roc(increasing = F, labels = FALSE, n.cuts = 0) +
         xlab("") + ylab("") + style_roc(theme = theme_grey)
